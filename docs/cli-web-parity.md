...
| CLI / TUI | Web surface |
| --- | --- |
| Onboarding wizard (first-run `.trau.ini` setup; TUI *Re-run onboarding*) | **Add a project** (`/projects/new`, reached from the sidebar CONFIGURE group and the repo switcher's *Add a project* row) — a path → inspect → tracker → essentials → seed-sync → done wizard that registers the repo, writes the tracker and essentials config to the project layer, and seeds the issue store. Secrets are write-only (blank keeps the stored value); the tracker step is gated on a passing connection test. After that test passes and a binding is chosen, azure/linear/jira also offer an optional collapsed *Status mapping* section — the same grouping/pin editor the settings page carries, fed by `POST /api/v1/trackers/{provider}/status-options` because the repo is not registered yet — whose keys ride the project-tracker write. Skipping it writes none of them. Registration stays fail-closed: on an exposed bind it also needs `SERVE_ALLOW_REGISTER` (the 403 renders an in-app remediation callout, never a raw error). See ADR 0012. |
