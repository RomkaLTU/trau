...
| Pre-verify checklist — six numbered items, "Surface every failure visibly… Drive the new UI with the keyboard and with overlays open" | `build` | Lessons-ledger analysis of 2026-08-05: 67 of 187 first verify runs failed since 2026-07-15, and the 157 recent `lessons` rows (all `phase=verify`) cluster into these six cross-cutting patterns. `lessonQuery` cannot recall them — it matches lessons by ticket-title keyword overlap, and the patterns are behavioral, not topical. Revise the six items from a fresh analysis of that ledger. | Receipted (ledger analysis) |
