...
// ensureHubForInternal guarantees the serve hub the internal tracker provider
// depends on is reachable before the loop reads or writes an issue. It probes the
...
// cannot be brought up leaves the provider to fail its first call with a clear
// error rather than aborting the run here.
func ensureHubForInternal(ctx context.Context, cfg config.Config, stderr io.Writer) {
	healthURL := hubBaseURL(cfg) + webserver.APIPrefix + "/health"
	if probeHub(ctx, healthURL, cfg.ServeToken).isHub {
		return
	}
	if err := webserver.CheckExposure(cfg.ServeBind, cfg.ServeToken); err != nil {
		_, _ = fmt.Fprintf(stderr, "trau: the internal tracker needs the web hub, but it can't autostart (%v)\n", err)
		return
	}
	if !cfg.ServeAutostart {
		_, _ = fmt.Fprintf(stderr, "trau: the internal tracker needs the web hub — start it with `trau serve` or set SERVE_AUTOSTART=1\n")
		return
	}
	if err := spawnDetachedServe(); err != nil {
		_, _ = fmt.Fprintf(stderr, "trau: could not autostart the web hub for the internal tracker: %v\n", err)
		return
	}
	if !waitHubHealthy(ctx, healthURL, cfg.ServeToken) {
		_, _ = fmt.Fprintf(stderr, "trau: the web hub did not become ready in time; internal-issue operations may fail until it does\n")
	}
}
