...
**Worktree**:
The per-ticket linked git worktree a run works in when its Repo has `WORKTREES=1` — one checkout per ticket at `<WORKTREES_DIR>/<repo-name>/<ticket-id>`, sharing the Repo's objects and refs. The run provisions it (fetch, `git worktree add`, copy the gitignored files git cannot carry, then `WORKTREE_SETUP_CMD`), works entirely inside it, and the tree is removed when the ticket settles — merge, Reset, Requeue, or purge. It is *where* the work happens and never *whose* the work is: git, the agent phases and every tracked-file lookup follow the Worktree, while checkpoints, artifacts, events, presence and the Queue stay keyed by the Registered repo. With Worktrees on the user's own checkout is never inspected, stashed or reset. An Epic is serial inside — its sub-issues build on the epic branch one after another in a single process — so it takes exactly one Worktree, keyed by the epic id: every sub-issue and the epic's release run in it, it survives a hand-off to a human until the epic PR lands, and it is removed when the epic itself settles. A Folder repo ignores the key — a folder has no repository at its root to add a tree to (ADR 0044). Where the Repo sets `APP_START_CMD`, a Worktree also serves the target app: the hub allocates it the lowest free port at or above `WORKTREE_PORT_BASE`, starts the command in the tree at first need, and supervises it until the tree is removed, so browser verify drives that lane's own `http://localhost:<port>` — the branch under test — rather than the dev server on the user's checkout.
_Avoid_: clone, lane, workspace, sandbox, branch (that's the ref the tree holds), checkout (bare — that's the Registered repo's own tree)
...
**Spawn hold**:
A draining Queue that starts nothing while a runnable item waits, named by the gate holding it — an unresolved blocker, a loop already running in the Repo, a pending self-reload, a release, or a drain loop that stopped ticking at all. The hub says so once per hold episode in the feed and for as long as it lasts on the queue API, so a wait is never indistinguishable from a hang.
_Avoid_: stalled/stuck (that's only one of the gates), blocked (that's the item's tracker edges), paused (that's a Failure class), idle
...
**Status mapping**:
A repo's own answer to which section of the board each of its tracker's statuses files under (`AZURE_BOARD_STATES`, `LINEAR_BOARD_STATES`, `JIRA_BOARD_STATES`) — one grammar, `<name>=<group>` pairs over backlog | unstarted | started | done | canceled, edited in Settings against the names the tracker itself reports. It comes in two shapes and the difference is load-bearing: an **exhaustive** mapping (Azure DevOps, keyed on Board column) is the whole grouping once set, and a name it omits groups as unknown; an **overlay** (Linear, keyed on workflow state name; Jira, keyed on status name) records only the statuses a repo moves, and one it omits keeps the section the tracker's own metadata derives — the Linear state type, the Jira status category (ADR 0038, ADR 0039). On Jira a listed status is authoritative enough to override the won't-do/duplicate resolution nuance too. It drives grouping only — a status *write* still resolves a state through the `STATUS_*` pins.
_Avoid_: status override (collides with the `STATUS_*` pins, which are writes), board mapping (only one provider maps a board), state mapping (that's Azure DevOps's own column→state metadata)
...
**Draft an issue**:
The one-click act offered on an open research Report that opens an Inbox Interview seeded with the Report, always as a new Draft — never over an existing issue. The interview's first turn is grounded in the whole findings, so it grills for scope, product decisions and slicing instead of re-asking what the research settled; the visible thread opens on one short note naming and linking the Report. The Report is untouched — still approvable, still open to follow-ups — and each click drafts an independent issue.
_Avoid_: file it, convert to issue (the Report is not converted, it stays), promote, spin off
