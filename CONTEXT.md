...
**Status mapping**:
A repo's own answer to which section of the board each of its tracker's statuses files under (`AZURE_BOARD_STATES`, `LINEAR_BOARD_STATES`) — one grammar, `<name>=<group>` pairs over backlog | unstarted | started | done | canceled, edited in Settings against the names the tracker itself reports. It comes in two shapes and the difference is load-bearing: an **exhaustive** mapping (Azure DevOps, keyed on Board column) is the whole grouping once set, and a name it omits groups as unknown; an **overlay** (Linear, keyed on workflow state name) records only the states a repo moves, and a state it omits keeps the section its own Linear state type derives (ADR 0038, ADR 0039). It drives grouping only — a status *write* still resolves a state through the `STATUS_*` pins.
