...
An acceptance criterion the verify verdict could not settle either way — the deciding observation was never made (browser QA undriven, a tool missing, an environment unreachable) — as opposed to **satisfied** (checked and true) or **violated** (checked and false). Any grade trau does not recognise, a missing one included, reads as unverified, so an absent observation never passes for one that was made. It is a coverage gap rather than a defect: it never fails a slice on its own, but it does keep the merge with a human — the PR still opens and lists those criteria in its body, and the run waits for a manual merge whatever `AUTO_MERGE` says (docs/verify-checks.md).
_Avoid_: failed criterion (that's violated — checked and false), unchecked, skipped, pending, inconclusive
