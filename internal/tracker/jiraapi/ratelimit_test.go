...
// the site said the budget refills, so the caller waits the window out instead of
// recording a failure nobody can fix.
func TestExhaustedRetriesSurfaceTypedRateLimitError(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := New(srv.URL, "me@acme.com", "tok").Ping(context.Background())
