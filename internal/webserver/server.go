...
func (s *Server) apiHandler() http.Handler {
	...
	mux.HandleFunc(APIPrefix+"/projects/{project}/start-repo", s.handleProjectStartRepo)
	...
	mux.HandleFunc(APIPrefix+"/repos/{repo}/azure/create-options", s.handleAzureCreateOptions)
	mux.HandleFunc(APIPrefix+"/repos/{repo}/tracker/status-options", s.handleTrackerStatusOptions)
	...
	mux.HandleFunc(APIPrefix+"/repos/{repo}/queue/batches", s.handleQueueBatches)
	mux.HandleFunc(APIPrefix+"/repos/{repo}/queue/batches/{bid}", s.handleQueueBatch)
	mux.HandleFunc(APIPrefix+"/repos/{repo}/queue/batches/{bid}/start", s.handleQueueBatchStart)
	mux.HandleFunc(APIPrefix+"/repos/{repo}/queue/{id}", s.handleQueueItem)
	mux.HandleFunc(APIPrefix+"/repos/{repo}/queue/{id}/{action}", s.handleQueueItemAction)
	...
	mux.HandleFunc(APIPrefix+"/grill/{sid}/choose-proposal", s.handleGrillChooseProposal)
	mux.HandleFunc(APIPrefix+"/grill/{sid}/mcp", s.handleGrillMCP)
	mux.HandleFunc(APIPrefix+"/grill/{sid}/mcp/{member}", s.handleGrillMemberMCP)
	mux.HandleFunc(APIPrefix+"/grill/{sid}/mcp/{member}/{round}", s.handleGrillMemberMCP)
