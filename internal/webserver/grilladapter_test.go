...
func TestCodexGrillActivity(t *testing.T) {
	tests := []struct {
		...
		{
			name: "completed reasoning carries its summary",
			line: `{"type":"item.completed","item":{"id":"item_3","type":"reasoning","text":"**Analyzing color-position constraints**"}}`,
			want: `{"seq":0,"kind":"thinking","text":"**Analyzing color-position constraints**"}`,
		},
		{
			name: "a long summary is cut back",
			line: `{"type":"item.completed","item":{"id":"item_3","type":"reasoning","text":"` + strings.Repeat("x", grillActivityThinkingMax+20) + `"}}`,
			want: `{"seq":0,"kind":"thinking","text":"` + strings.Repeat("x", grillActivityThinkingMax) + `…"}`,
		},
		{name: "agent message", line: `{"type":"item.completed","item":{"type":"agent_message","text":"push back"}}`},
		{name: "reasoning started", line: `{"type":"item.started","item":{"id":"item_3","type":"reasoning"}}`},
		{name: "turn completed", line: `{"type":"turn.completed","usage":{"input_tokens":1}}`},
		{name: "not json", line: `warning: ignore me`},
		{name: "blank"},
	}
