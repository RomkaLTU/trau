...
// ingestPass tails every known repo's token files into the derived tables.
// Ingestion is best-effort per repo and per source: a torn line, rewritten file,
// or read error is skipped or resynced, never propagated, so it can never take the
// hub down (ADR 0007 §3). Checkpoints and events are no longer tailed — the loop
// writes them straight to their authoritative tables over HTTP (ADR 0008).
