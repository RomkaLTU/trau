...
func KnownKeys() []KeyMeta {
	keys := []KeyMeta{
		...
		{Key: "REQUIRED_SKILLS", Group: sectionSkills, WebEditable: true, Description: "Skill names (comma-separated) the build, repair and bugfix agents must load. Names not installed in the repo warn at loop start. Empty = the project type's recommended skills, or every installed skill when none match"},
		{Key: "REQUIRED_SKILLS_VERIFY", Group: sectionSkills, WebEditable: true, Description: "Skill names (comma-separated) the verify agent must load, alongside the project's test skills and browser-harness on browser verify. Empty = verify loads the build set"},
