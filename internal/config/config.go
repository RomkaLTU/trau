...
type Config struct {
	...
	// implementing (config REQUIRED_SKILLS, comma-separated). It is the first
	// step of the build/repair/bugfix resolution chain; when it is empty the
	// chain falls back to the project type's recommended set, then to every
	// installed skill. Names not installed in the repo warn at loop start.
	RequiredSkills []string
	// RequiredSkillsVerify names the skills the verify agent must load before
	// checking a slice (config REQUIRED_SKILLS_VERIFY, comma-separated). It joins
	...
	// resolved set, for the agent to load on its own judgment. It applies only to
	// claude in instruct mode — the other providers have no Skill tool.
	SkillsMenu bool
	...
	VerifyProofs string
	// ProofRetentionDays bounds how long the hub keeps a run's raw local trace and
	// its trau-proofs branch images before the daily sweep prunes them
	// (PROOF_RETENTION_DAYS key). 0 keeps them forever; hub-stored screenshots and
	// rendered videos are small and always kept.
	...
	// UpdateCheck lets the hub ask GitHub once a day whether a newer trau
	// release exists. Off, no request is ever made; drift between the running
...
func LoadLayeredWithSources(projectPath, userPath, localPath, provider string) (Config, map[string]Layer, error) {
	...
	if v, src := get("SKILLS_MENU"); v != "" {
		c.SkillsMenu = v == "1"
		sources["SKILLS_MENU"] = src.name
	}
	...
	num("PROOF_RETENTION_DAYS", &c.ProofRetentionDays)
...
func KnownKeys() []KeyMeta {
	...
	keys := []KeyMeta{
		...
		{Key: "AUTO_MERGE", Group: sectionGit, WebEditable: true, Default: "1", Description: "Merge on green CI (1 = auto-merge, 0 = wait for manual merge). A slice whose verdict left an acceptance criterion unverified always waits for a manual merge, whichever way this is set", Bool: true},
...
func ProviderTuningMetas() []ProviderTuningMeta {
	return []ProviderTuningMeta{
		{
			Name:    "claude",
			Models:  []string{"fable", "claude-opus-5", "claude-sonnet-5", "opus", "sonnet", "haiku"},
...
func keyValue(cfg Config, key string) string {
	...
	case "PROOF_RETENTION_DAYS":
		return intValue(cfg.ProofRetentionDays)
	case "APP_URL":
		return cfg.AppURL
	case "APP_URLS":
