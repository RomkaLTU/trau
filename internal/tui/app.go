...
func (m appModel) handleRunningKey(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	// While the dash filter input is capturing, every key belongs to it — route
	// straight to the dash so an action key (q, o, j) narrows the feed instead of
	// firing the action or moving the rail.
