...
// isRepoPickerShortcut matches the ⌘P / Ctrl+P repo picker. Browser print rides
// the same chord, so callers preventDefault it.
export function isRepoPickerShortcut(e: PaletteKeyEvent): boolean {
  if (e.isComposing) return false
  if (!e.metaKey && !e.ctrlKey) return false
...
export function shortcutLabel(mac: boolean, key = 'K'): string {
