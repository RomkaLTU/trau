...
describe("notificationTarget", () => {
  it("sends a grill question to its inbox issue in its repo", () => {
...
describe("notificationRepoName", () => {
  const repos = [repoView("app"), repoView("api")];

  it("resolves a persisted-row repo root to its registry name", () => {
    expect(notificationRepoName("/repos/api", repos)).toBe("api");
  });

  it("passes a live-frame registry name through", () => {
    expect(notificationRepoName("api", repos)).toBe("api");
  });
...
describe("notificationScopeSwitch", () => {
  const repos = [repoView("app"), repoView("api")];

  it("adopts a cross-project inbox target repo from a live frame (name)", () => {
    expect(
      notificationScopeSwitch(
        { kind: "inbox", repo: "api", issue: "COD-1" },
        "app",
        repos,
      ),
    ).toBe("api");
  ...
  it("adopts a cross-project inbox target repo from a persisted row (root)", () => {
    expect(
      notificationScopeSwitch(
        { kind: "inbox", repo: "/repos/api", issue: "COD-1" },
        "app",
        repos,
      ),
    ).toBe("api");
  ...
  it('adopts the target repo under "All projects"', () => {
    expect(
      notificationScopeSwitch(
        { kind: "inbox", repo: "/repos/api", issue: "COD-1" },
        null,
  ...
  it("stays put for a same-repo target addressed by name", () => {
    expect(
      notificationScopeSwitch(
        { kind: "inbox", repo: "app", issue: "COD-1" },
        "app",
        repos,
      ),
    ).toBeNull();
  ...
  it("stays put for a same-repo target addressed by root", () => {
    expect(
      notificationScopeSwitch(
        { kind: "inbox", repo: "/repos/app", issue: "COD-1" },
        "app",
        repos,
      ),
    ).toBeNull();
  ...
  it("skips a repo the hub no longer lists", () => {
    expect(
      notificationScopeSwitch(
        { kind: "inbox", repo: "gone", issue: "COD-1" },
        "app",
        repos,
      ),
    ).toBeNull();
    expect(
      notificationScopeSwitch(
        { kind: "inbox", repo: "/repos/gone", issue: "COD-1" },
        "app",
        repos,
      ),
    ).toBeNull();
  ...
  it("never switches for a run target", () => {
    expect(
      notificationScopeSwitch(
        { kind: "run", repo: "/repos/api", ticket: "COD-9" },
        "app",
        repos,
      ),
    ).toBeNull();
